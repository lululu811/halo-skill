...
def generate_skeleton(json_path, qual_path=None):
    ...
    w("{{AI_BIZ_TABLE}}")
    w("")
    w("> 📝 **AI 解读**（基于 1.2 业务结构表，不得编造或修改数字）：")
    w(">")
    w("> - **业务集中度**：第一板块占比是否过高（单一业务风险）或多元（分散风险）")
    w("> - **毛利率水平**：结合行业判断盈利能力与护城河强弱")
    w("> - **核心产品竞争力**：是否行业龙头/差异化定位")
    w("")
    w("{{AI_BIZ_ANALYSIS}}")
    w("")
    w("### ⭐ 1.3 核心竞争优势\n")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 3.1 表，不得编造或修改数字）：")
    w(">")
    w("> - 结合有形资产密集度评分说明资产属性（重/混合/轻）与行业匹配度")
    w("> - 在建工程与存货占比的含义（扩张期/行业存货特性）")
    w("")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 3.2 表，不得编造或修改数字）：")
    w(">")
    w("> - 固定资产相对营收的比重反映产能规模效应")
    w("> - 结合资产类型说明评分合理性（轻资产低分不应被过度惩罚）")
    w("")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 3.3 表，不得编造或修改数字）：")
    w(">")
    w("> - 固定资产在总资产中的地位（资产结构特征）")
    w("> - 结合行业属性说明份额水平是否合理")
    w("")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 3.4 表，不得编造或修改数字）：")
    w(">")
    w("> - 人均装备水平反映资本密集型 vs 劳动密集型")
    w("> - 员工规模与资产匹配度（是否人效合理）")
    w("")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 3.5 表，不得编造或修改数字）：")
    w(">")
    w("> - 资本开支强度反映扩张意愿或维护需求")
    w("> - 结合行业判断 capex 水平是否合理（重资产高 capex）")
    w("")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 3.6 表，不得编造或修改数字）：")
    w(">")
    w("> - Capex 对经营现金流的消耗程度（负担轻重）")
    w("> - 过度投资风险或轻装上阵优势")
    w("")
    ...
    w("> 📝 **AI 解读**（基于 4.1 营收表，不得编造或修改数字）：")
    w(">")
    w("> - **增速质量**：单季同比与年度估增速是否一致，增速是否可持续")
    w("> - **增速归因**：量价拆解、新品/并购/周期因素")
    w("> - **评分合理性**：增速对应评分是否符合阈值逻辑")
    w("")
    w("{{AI_REVENUE_ANALYSIS}}")
    w("")
    w("### 💰 4.2 利润增长\n")
    w("| 📊 期间 | 净利润 | 同比 |")
    w("|:--------|:------:|:----:|")
    ...
    w("> 📝 **AI 解读**（基于 4.2 利润表，不得编造或修改数字）：")
    w(">")
    w("> - **利润 vs 营收增速**：利润增速快于/慢于营收，说明经营杠杆或成本挤压")
    w("> - **增速质量**：经常性利润 vs 一次性损益（政府补助/资产处置等）")
    w("> - **评分合理性**：增速对应评分是否符合阈值逻辑")
    w("")
    w("{{AI_PROFIT_ANALYSIS}}")
    w("")
    w("### 🔍 4.3 增长质量\n")
    w("| 📊 评估项 | ⭐ 评分 | 📝 说明 |")
    w("|:----------|:------:|:--------|")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 7.1/7.2 上表数据，不得编造或修改数字）：")
    w(">")
    w("> - **治理(G)**：结合分红连续性与最近股息率，评估股东回报政策的稳定性与诚意；处罚记录数据缺失，结合企业性质（国企/民企）定性说明合规风险")
    w("> - **环境(E)/社会(S)**：结合所属行业的典型 ESG 议题（如白酒的高耗水/包装、化工的排放、制造的能耗）说明企业应对")
    w("> - **ESG 综合评分(0-10)**：给出建议分数与核心理由")
    w("")
    w("{{AI_ESG_ANALYSIS}}")
    w("")
    w("---")
    ...
    w("")
    w("> 📝 **AI 解读**（基于 8.1 上表数据，不得编造或修改数字）：")
    w(">")
    w("> - **管理层稳定性**：董事长/总经理任职情况，是否有代行、频繁变动等异常")
    w("> - **组织规模匹配度**：员工人数与管理人员数是否匹配业务规模与行业惯例")
    w("> - **资本配置历史**：结合分红记录（见 9.2/7.1）评估管理层资本配置能力与股东友好度")
    w("> - **管理层评分(0-10)**：给出建议分数与核心理由")
    w("")
    w("{{AI_MGMT_ANALYSIS}}")
    w("")
    w("---")
    ...
    w(">")
    w("> 我们跟踪：股东人数变化（筹码集中度）、分红回报（股东友好度）、主力资金（大单流向）。")
    w("> 北向资金实时数据自2024年8月起暂停披露，融资融券接口待接入，相关项标注缺失。")
    w(">")
    ...
    # 9.1 股东人数与筹码集中度
    w("### 👥 9.1 股东人数与筹码集中度\n")
    _holder_num = d.get("holder_num", [])
    if _holder_num:
        _h0 = _holder_num[0]
        _hn = _h0.get("holder_num", 0)
        _hc = _h0.get("holder_num_change", 0)
        _hd = str(_h0.get("date", ""))[:10]
        _trend = "集中（主力收集）" if _hc < 0 else "分散（筹码松动）"
        w("| 📊 期间 | 股东人数 | 较上期变化 | 筹码趋势 |")
        w("|:--------|:--------:|:----------:|:--------:|")
        w(f"| {_hd} | {int(_hn):,} | {_hc:+.0f} | {_trend} |")
        w("")
        w("> 💡 股东人数减少通常意味筹码集中（主力收集），增加意味筹码分散。")
    else:
        w("> ⚠️ 缺失：无股东人数数据")
    w("")
    ...
    # 9.2 分红回报
    w("### 💵 9.2 分红回报\n")
    _dividend = d.get("dividend", [])
    _impl = [x for x in _dividend if x.get("progress") == "实施分配" and x.get("pretax_bonus", 0) > 0]
    if _impl:
        _ld = _impl[0]
        w("| 📊 分红方案 | 股息率 | 除权除息日 |")
        w("|:-----------|:------:|:----------:|")
        w(f"| {_ld.get('plan_profile','?')} | {_ld.get('dividend_ratio',0)*100:.2f}% | {str(_ld.get('ex_dividend_date',''))[:10]} |")
        w("")
        if len(_impl) > 1:
            _recent = _impl[:5]
            w(f"> 📊 近{len(_recent)}次分红：{'；'.join(x.get('plan_profile','?') for x in _recent)}")
    else:
        w("> ⚠️ 缺失：无已实施分红记录")
    w("")
    ...
    # 9.4 北向资金与融资融券（数据源暂不可用）
    w("### 🌐 9.4 北向资金与融资融券\n")
    w("> ⚠️ 数据源暂不可用：北向资金实时数据自2024年8月起暂停披露；融资融券接口待接入。")
    w("")

    # AI 综合分析槽位
    w("> 📝 **AI 解读**（基于 9.1-9.4 上表数据，不得编造或修改数字）：")
    w(">")
    w("> - **筹码集中度**：结合股东人数变化方向（集中/分散）说明主力动向含义")
    w("> - **分红回报**：结合分红连续性与股息率水平评估股东友好度")
    w("> - **主力资金**：近5日主力净流入是短期信号，不代表趋势，需结合基本面")
    w("> - **数据缺失说明**：北向/融资暂不可用，对资金面判断的影响")
    w("> - **资金面评分(0-10)**：给出建议分数与核心理由")
    w("")
    w("{{AI_SHAREHOLDER_ANALYSIS}}")
    w("")
    w("---")
    w("")
    ...
    if _val:
        ...
        w("> ⚠️ 缺失：无估值分位数据")
        w("")
    w("> 📝 **AI 解读**（基于上表 PE/PB/PS/PCF 分位数据，不得编造或修改数字）：")
    w(">")
    w("> - **估值分位一致性**：四指标分位是否指向同一方向（均低/均高/分化），分化时说明什么")
    w("> - **历史位置**：当前估值在过去历史中处于便宜/合理/偏贵区间")
    w("> - **估值与增速匹配**：结合成长性评分（第四章）判断 PEG 思路下是否合理")
    w("> - **估值吸引力评分(0-10)**：给出建议分数与核心理由")
    w("")
